...
import { AppAiDescriptionInputSchema, AppAiPriceInputSchema } from "./src/ai-validation.ts"
import { generateDescription, generatePrice } from "./src/groq-ai-repository.ts"
...
export async function createApp() {
  ...
  fastify.post("/ai/generate-description", async (request, reply) => {
    try {
      const { item } = AppAiDescriptionInputSchema.parse(request.body)
  ...
  fastify.post("/ai/generate-price", async (request, reply) => {
    try {
      const { item } = AppAiPriceInputSchema.parse(request.body)
